...
export default function OfficerLayout({ children }: { children: ReactNode }) {
  ...
  const navigation: NavigationItem[] = [
    ...
    { name: 'marketplace', href: '/officer/marketplace', icon: ShoppingBagIcon },
    { name: 'applications', href: '/officer/applications', icon: ClipboardDocumentListIcon },
    { name: 'citizens', href: '/officer/citizens', icon: UserGroupIcon },
    { name: 'notifications', href: '/officer/notifications', icon: BellIcon },
    { name: 'security', href: '/officer/security', icon: ShieldCheckIcon },
    ...
    <div className={cn('bg-gray-900', 'bg-gray-50') + ' min-h-screen'}>
      ...
      <div className={cn('bg-gray-800 border-gray-700', 'bg-white border-gray-200') + ' fixed inset-y-0 left-0 w-64 border-r transition-colors duration-200'}>
        <div className={cn('border-gray-700', 'border-gray-200') + ' flex items-center h-16 px-6 border-b'}>
          ...
          <span className={cn('text-white', 'text-gray-900') + ' ml-3 font-semibold'}>{t('officerPortal')}</span>
        </div>
        ...
        <nav className="mt-6 px-3">
          {navigation.map((item) => {
            ...
            const linkClass = active 
              ? cn('bg-blue-900/30 text-blue-400', 'bg-blue-50 text-blue-700') + ' flex items-center px-4 py-3 mb-1 text-sm rounded-lg'
            ...
            const iconClass = active
              ? cn('text-blue-400', 'text-blue-600') + ' w-5 h-5 mr-3'
        ...
        <header className={cn('bg-gray-800 border-gray-700', 'bg-white border-gray-200') + ' border-b h-16 flex items-center justify-between px-8 transition-colors duration-200'}>
          ...
          <div className="flex items-center space-x-4">
            <select
              ...
              ) + ' border rounded-lg text-sm px-3 py-1.5 focus:outline-none focus:ring-2 focus:ring-blue-500'}
            >
            ...
            <div className="flex items-center space-x-3">
              <span className={cn('text-gray-300', 'text-gray-700') + ' text-sm'}>Officer</span>
              <img
                src="https://ui-avatars.com/api/?name=Land+Officer&background=2563eb&color=fff"
                alt="Officer"
